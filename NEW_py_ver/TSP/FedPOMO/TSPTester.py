...
class TSPTester:
    def __init__(self,
        ...
        model_type = model_load.get('type', 'checkpoint')  # 默认为 'checkpoint'
        # 根据指定的类型构建文件名
        if model_type == 'globalModel':
            checkpoint_fullname = '{path}/globalModel-{epoch}.pt'.format(**model_load)
        else:
            checkpoint_fullname = '{path}/checkpoint-{epoch}.pt'.format(**model_load)        
        checkpoint = torch.load(checkpoint_fullname, map_location=device)
        # 检查 'model_state_dict' 键是否在 checkpoint 中，并据此加载模型状态
        if 'model_state_dict' in checkpoint:
            # 如果存在 'model_state_dict' 键，则使用它的值来加载模型状态,,此时测试的是POMO训练出的单个模型的checkpoint.pt
            self.model.load_state_dict(checkpoint['model_state_dict'])
        else:
            # 如果不存在 'model_state_dict' 键，则直接使用整个检查点来加载模型状态,此时测试的是globalModel.pt
